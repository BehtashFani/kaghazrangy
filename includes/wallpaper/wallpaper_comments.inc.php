...
<?php
defined( 'WSS_' ) or die( '' );
$sql = mysql_query("SELECT * FROM wss_comments WHERE link_id=".$id." ORDER BY id desc");
while ($row = mysql_fetch_array($sql)) {
	...
	if ($user['admin'] == 1) { 
		$comment['delete'] = '<a href="#" onclick="DeleteComment('.$row['id'].', '."'".$setting['site_url']."'".'); return false">'.DELETE_COMMENT.'</a>';
